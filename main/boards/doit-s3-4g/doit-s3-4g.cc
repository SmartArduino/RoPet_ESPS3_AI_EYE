...
class DoitMl307Board : public Ml307Board {
private:
    ...
    void Enable4GModule() {
        // Make GPIO36 HIGH to enable the 4G module
    ...
    void InitializeCodecI2c() {
        ...
        ESP_ERROR_CHECK(i2c_new_master_bus(&i2c_bus_cfg, &codec_i2c_bus_));
         ...
         // Initialize PCA9557
         //pca9557_ = new Pca9557(codec_i2c_bus_, 0x19);    
    }
...
public:
    DoitMl307Board() : Ml307Board(ML307_TX_PIN, ML307_RX_PIN, 4096),
        boot_button_(BOOT_BUTTON_GPIO),
        touch_button_(TOUCH_BUTTON_GPIO),
        volume_up_button_(VOLUME_UP_BUTTON_GPIO),
        volume_down_button_(VOLUME_DOWN_BUTTON_GPIO) {
        ESP_LOGI(TAG,"开始初始化音频编码器");
        InitializeCodecI2c();
        ESP_LOGI(TAG,"开始初始化I2C");
         InitializeDisplayI2c();
         ESP_LOGI(TAG,"开始初始化Display");
         InitializeSsd1306Display();
         ESP_LOGI(TAG,"开始初始化WS2812");
        // InitializeSpi();
        // InitializeIli9341Display();

        // ESP_LOGI(TAG,"使能4G模块");
        // Enable4GModule();
        ESP_LOGI(TAG,"开始初始化Buttons");
        InitializeButtons();

        //InitializePowerSaveTimer();

        ESP_LOGI(TAG,"开始初始化IOT");
        InitializeIot();
    }
