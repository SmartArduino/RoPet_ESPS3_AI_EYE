...
Application::Application() {
    ESP_LOGI(TAG,"Firmware burning was successful!!!.");
...
void Application::Start() {
...
#if CONFIG_USE_WAKE_WORD_DETECT //唤醒词
    wake_word_detect_.Initialize(codec);    //初始化编码器
    wake_word_detect_.OnWakeWordDetected([this](const std::string& wake_word) { //当唤醒词被检测到时会触发。回调函数的参数是一个 std::string 类型的 wake_word，表示检测到的唤醒词内容。
        Schedule([this, &wake_word]() {
            if (device_state_ == kDeviceStateIdle) {
                SetDeviceState(kDeviceStateConnecting);
                wake_word_detect_.EncodeWakeWordData(); //这行代码将检测到的唤醒词数据进行编码，通常会将音频数据转换为适合传输的格式

                if (!protocol_->OpenAudioChannel()) {   //打开音频编码器
                    wake_word_detect_.StartDetection(); //开始检测唤醒词
                    return;
                }
    ...
    SetDeviceState(kDeviceStateIdle);
    esp_timer_start_periodic(clock_timer_handle_, 1000000); //一秒的定时
...
void Application::OnClockTimer() {
    ...
    // Print the debug info every 10 ms 
}

// Add a async task to MainLoop
//为主循环添加异步任务，该函数通过事件组的方式让BackgroupTask执行对应的函数
void Application::Schedule(std::function<void()> callback) {
    {
        std::lock_guard<std::mutex> lock(mutex_);
        main_tasks_.push_back(std::move(callback)); //添加任务到main_tasks中，然后BackendgroundTask任务会自己去调用
    }
...
//主循环控制聊天状态和websocket连接如果其他任务需要访问websocket或聊天状态，他们应该使用Schedule来调用这个函数
