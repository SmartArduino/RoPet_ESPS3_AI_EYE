...
static jpeg_error_t decode_jpg_from_mem(const uint8_t *jpg_data, const int jpg_len, lv_image_dsc_t *output_dsc) {
    // DV_LOGI( "decode open： %ld", (uint32_t)esp_timer_get_time());
    ...
    // DV_LOGW("JPG decode size: %dx%d", out_info.width, out_info.height);
    ...
    // DV_LOGI( "img decode success %dx%d  %lu bytes", out_info.width, out_info.height, output_dsc->data_size);

    // DV_LOGI( "decode over： %ld", (uint32_t)esp_timer_get_time());
    return ret;
}
...
static void vpg_show_task(void *pvParameters) {
    ...
    while (true) {
        show_idle = true;                  // 空闲状态
        if (!vpg || !vpg->is_vpg_player) { /* 停止期让出 CPU */
...
static void vpg_decode_task(void *pvParameters) {
    while (true) {
        ...
        // DV_LOGI( "current frame %d", vpg->cur_frame_idx);
        ...
        if (JPEG_ERR_OK == decode_jpg_from_mem(vpg->psram_data, cur_fr_size, &vpg->lv_buf[idx])) {
            ...
            /* 解码失败，直接归还缓冲区 */
            DV_LOGE("解码失败，跳过第 %d 帧", vpg->cur_frame_idx);
        ...
        if (++vpg->cur_frame_idx >= vpg->fileHeader.itemNum)
            vpg->cur_frame_idx = 0; // 循环播放
                                    // DV_LOGI( "===decode over===： %ld", (uint32_t)esp_timer_get_time());
                                    // DV_LOGI("vpg->fileHeader.fps=%lu",vpg->fileHeader.fps);
...
void inc_vpg_player_stop(void) {
    ...
    if ((!decode_idle || !show_idle)) {
        DV_LOGW("stop timeout, force suspend");
    }
    ...
    if (ret != 0) {
        DV_LOGE("fclose error");
    }
    vpg->fp = NULL; // 防止后续误用

    DV_LOGI("VPG player stopped,Free PSRAM: %d,min free sram=%d", heap_caps_get_free_size(MALLOC_CAP_SPIRAM), heap_caps_get_minimum_free_size(MALLOC_CAP_SPIRAM));
}
...
void inc_vpg_player_start(const char *dir_name) {
    DV_LOGI("加载的动画文件是：%s", dir_name);
    // if (vpg && vpg->is_vpg_player)
    // {
    //     dot_vpg_player_stop();
    // }
    DV_LOGI("dot_vpg_player_start:psram可用大小=%d,psram可用的最大块=%d", heap_caps_get_free_size(MALLOC_CAP_SPIRAM), heap_caps_get_largest_free_block(MALLOC_CAP_SPIRAM));
    if (!vpg_is_init) {
        // 初始化vpg
        if (!vpg) {
            // 1. 分配vpg结构体内存
            ...
            vpg->img_obj = page_manager_get_home_img_obj();

            for (uint16_t i = 0; i < VPG_BUFF_NUM; ++i) {
                if (LV_VER_RES == 360 && LV_HOR_RES == 360) {
                    vpg->lv_buf[i].data = (uint8_t *)heap_caps_aligned_alloc(16, // 对齐单位
                                                                             368 * 368 * 2,
                                                                             MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
                } else {
                    vpg->lv_buf[i].data = (uint8_t *)heap_caps_aligned_alloc(16, // 对齐单位
                                                                             LV_VER_RES * LV_HOR_RES * 2,
                                                                             MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
                }

                if (!vpg->lv_buf[i].data) {
                    DV_LOGE("PSRAM malloc failed for lv_buf[%d]", i);
                    return;
                }
            }
            ...
            if (ret != pdPASS)
                DV_LOGE("show task create fail, ret=%d", ret);
            else {
                DV_LOGI("show task create ok, handle=%p", s_vpg_show_task);
            ...
            if (ret != pdPASS)
                DV_LOGE("decode task create fail, ret=%d", ret);
            else {
                DV_LOGI("decode task create ok, handle=%p", s_vpg_decode_task);
                vTaskSuspend(s_vpg_decode_task); // 先挂起
            }

            DV_LOGI("VPG player started,Free PSRAM: %d", heap_caps_get_minimum_free_size(MALLOC_CAP_SPIRAM));
    ...
    vpg->cur_frame_idx = 0;
    snprintf(vpg->spiff_path, 32, dir_name);
    ...
    if (vpg->fp == NULL) {
        /* code */
        DV_LOGE("文件读取失败,读取的路径是：%s", vpg->spiff_path);
        DV_LOGE("fopen 失败: %s (errno=%d)", strerror(errno), errno);
        return;
    }
    ...
    DV_LOGI("【VPG】文件总大小=%lu 字节", vpg->psram_size);

    // 读文件头
    if (fread(&vpg->fileHeader, sizeof(FileHeader), 1, vpg->fp) != 1) {
        DV_LOGE("fileHeader read fail");
        return;
    }
    if (vpg->fileHeader.magic != 0xAABBCCDD) {
        DV_LOGE("【VPG】魔数错误，不是合法 VPG 文件");
        return;
    }
    ...
    if (!vpg->ItemHeader) {
        DV_LOGE("vpg->ItemHeader heap_caps_malloc fail");
        return;
    }
    if (fread(vpg->ItemHeader, idx_bytes, 1, vpg->fp) != 1) {
        DV_LOGE("vpg->ItemHeader read index fail");
        return;
    }
    ...
    DV_LOGI("本次一帧最大的大小是=%lu", max_frame_size);
    ...
    // {
    //     DV_LOGI( "【VPG】第 %d 帧，大小=%lu 字节, 偏移=%lu", i, vpg->ItemHeader[i].size, vpg->ItemHeader[i].offset);
    //     DV_LOG_BUFFER_HEX("【VPG】", vpg->psram_data + vpg->ItemHeader[i].offset, 10);
    // }
