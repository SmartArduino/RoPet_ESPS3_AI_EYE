...
static void onCreate(page_t *page) {
    ...
    lv_label_set_text(s_label, "网络配置中,请连接xiaozhi开头的热点配置网络");
...
static void onShow(page_t *page) {
    ...
    if (s_label) {
        strncpy(s_label_text, (char *)page->stash->ptr, TOAST_TEXT_MAX - 1);
        s_label_text[TOAST_TEXT_MAX - 1] = '\0'; // 确保字符串以 null 字符结尾
        lvgl_port_lock(-1);
